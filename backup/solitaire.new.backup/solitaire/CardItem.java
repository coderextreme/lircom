...
public class CardItem extends JLabel implements ListCellRenderer, MouseListener, Cloneable {
	...
	static ImageIcon background = null; // = new ImageIcon(cardset+"back101.gif");
	...
	public CardItem(int suit, int rank, boolean faceUp, int stack, int position) {	
	        if (background == null) {
			background = new ImageIcon(this.getClass().getClassLoader().getResource(cardset+"back101.gif"));
		}
		this.suit = suit;
		this.rank = rank;
		this.faceUp = faceUp;
	...
	public void setIcon(String gifname) {
		ii // = new ImageIcon(gifname);
                      = new ImageIcon(this.getClass().getClassLoader().getResource(gifname));
		setIcon(ii);
	...
	public void setFaceUp(boolean faceUp) {
		if (!this.faceUp && faceUp) {
			if (stack == -1) {
				GameMiner.processLine("VISIBLE|0|0|"+toString(), null, this);
			} else {
				GameMiner.processLine("VISIBLE|"+stack+"|"+position+"|"+toString(), null, this);
			}
			Log.write(Stack.cards);
		} else if (this.faceUp && !faceUp) {
			if (stack == -1) {
				GameMiner.processLine("INVISIBLE|0|0|"+toString(), null, this);
			} else {
				GameMiner.processLine("INVISIBLE|"+stack+"|"+position+"|"+toString(), null, this);
			}
			Log.write(Stack.cards);
		}
		this.faceUp = faceUp;
