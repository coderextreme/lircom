...
class FromServer extends OutputStream {
	...
	public void write(int b) {
		// System.err.println("wrote "+b);
                Message m = new Message("Tiny Byte", "Tiny Byte", b+"");
...
public class Chat extends ClientOnServer implements WindowListener, ActionListener, FocusListener, ReceiveChatInterface {
	...
	StringBuffer html = new StringBuffer("<html><head><title>Chat</title></head><body><font color='white'><h1>Start Chatting</h1></font><div align='left'></div></body></html>");
	String color = "white";
	...
	class ProcessCommand implements ActionListener {
		...
		if (sci != null) {
	        ...
	        Message m = new Message(rec, from, line);
		try {
	...
	public void say(String nick, String verb, String message, String color) {
		...
		if (verb.equals("says")) {
			addToEnd("<div align='left'><font color='"+color+"'>&lt;"+nick+"&gt;"+smilies.replace(message)+"</font></div>");
		} else {
			addToEnd("<div align='left'><font color='"+color+"'>"+nick+" "+verb+" "+message+"</font></div>");
		}
	}
	...
	public void windowClosing(WindowEvent we) {
                Message m = new Message("*", getNick(), "has exited");
                try {
		    Hashtable rec = prepareToSend(m);
                    send(m, rec);
                } catch (Message msge) {
