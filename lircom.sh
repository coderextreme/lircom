...
${JAVA_OPTS} lircom.Peer ${PORTS} | \
	...
	xargs -L ${NUM_PORTS} ${JAVA_OPTS} -jar target/lircom-3-jar-with-dependencies.jar lircom.Chat Julie | \
	xargs -L ${NUM_PORTS} ${JAVA_OPTS} -jar target/lircom-3-jar-with-dependencies.jar lircom.Chat Kathy | \
